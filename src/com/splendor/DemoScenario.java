...
public class DemoScenario {
    ...
    public static void main(String[] args) {
        ...
        // Give both players 15 prestige points to trigger the tiebreaker end-game scenario
        player1.addPrestigePoints(15);
        player2.addPrestigePoints(15);

        // Give Player 1 more cards than Player 2 so that Player 2 wins the tiebreaker
        // Taking some dummy cards out of the tier 1 deck to make it realistic
        for (int i = 0; i < 4; i++) {
            if (!tier1.isEmpty()) player1.addOwnedCard(tier1.remove(0));
        }

        for (int i = 0; i < 2; i++) {
            if (!tier1.isEmpty()) player2.addOwnedCard(tier1.remove(0));
        }

        System.out.println("\n[DEMO] Scenario injected: Both players are at 15 points!");
        System.out.println("[DEMO] Player 1 has " + player1.getOwnedCards().size() + " cards.");
        System.out.println("[DEMO] Player 2 has " + player2.getOwnedCards().size() + " cards (Wins tiebreaker).");
        System.out.println("[DEMO] The game will end right after this first round completes.\n");
        ...
        System.out.println("\nSplendor setup complete! Starting the DEMO game.\n");
